...
if (file_exists($_SESSION['filedb'])){
	...
	header('location:banco.php?ex=rel_diabeticos');
}
if (file_exists("cfg_rel_d.php")){
	require_once('cfg_rel_d.php');
} else {
	header('location:diabeticos.php?ex=rel_diabeticos');
}
...
// +++++++++++++++++++++++++++++++++++++++++++++
$file = "csv/diab_T_01.csv";
if (file_exists($file)){unlink($file);}
$FT = fopen($file,'w');
$file = "csv/diab_C_01.csv";
if (file_exists($file)){unlink($file);}
$FC = fopen($file,'w');
$file = "csv/diab_P_01.csv";
if (file_exists($file)){unlink($file);}
$FP = fopen($file,'w');
// +++++++++++++++++++++++++++++++++++++++++++++
...
<head>
  ...
  <title>Relatório de diabeticos</title>
...
$num_diabeticos = 0;
$run_s1 = pg_query($cdb,$diabeticos);
$num_diabeticos = pg_num_rows($run_s1);
if ($num_diabeticos > 0){
	while ($diabetico = pg_fetch_array($run_s1)){
		$CNS = '0';
		$CPF = '0';
		$campo_busca = "";
		$campo_ordem = "";
		...
		$busca_cpf = false;
		$busca = false;
		if (strlen(trim($diabetico['nu_cns'])) == 15){
			$busca_cns = true;
			$CNS = trim($diabetico['nu_cns']);
		}
		if (strlen(trim($diabetico['nu_cpf_cidadao'])) == 11){
			$busca_cpf = true;
			$CPF = trim($diabetico['nu_cpf_cidadao']);
		}
		...
		if ($busca){
			...
			$cidadao_nascimento = $diabetico['dt_nascimento'];
			$cidadao_obito = NULL;
			$cidadao_faleceu = 0;
			$cidadao_cns_res = '0';
			$cidadao_cpf_res = '0';
			...
			if (!$duplicado){
				$data_nascimento = dataint($diabetico['dt_nascimento']); // ou dataint($cidadao_nascimento)
				$data_falecimento = "0";
...
$num_diabeticos_final = 0;
$run_hi = pg_query($cdb,$diabeticos_final);
$num_diabeticos_final = pg_num_rows($run_hi);
$conta_geral = $pini;
$conta_geral_desconto = $cdes;
$numerador_ind7 = $num;

if ($num_diabeticos_final > 0){
	while ($diabetico_final = pg_fetch_array($run_hi)){
		...
		$rCNS = trim($diabetico_final['cns']);
		$rCPF = trim($diabetico_final['cpf']);
		...
		if ($grupo != "SG"){
			$grupo_id = $diabetico_final[$grupo];
			
			$grupo_nome = "Micro-Área";
			$show_gp_inverso = "Unidade";	
			$show_gp_id_inverso = $diabetico_final['cnes'];	
			$show_gp_nm_inverso = $diabetico_final['nome_unidade'];
			$show_gp_id2_inverso = $diabetico_final['ine'];
			$show_gp_nm2_inverso = "Equipe";
			if ($grupo == "ine"){
				$grupo_nome = $diabetico_final['nome_equipe'];	
				$show_gp_id2_inverso = $diabetico_final['cind_micro_area'];
				$show_gp_nm2_inverso = "Micro-&Aacute;rea";
			}
			if ($grupo == "cnes"){
				$grupo_nome = $diabetico_final['nome_unidade'];
				$show_gp_inverso = "Equipe";	
				$show_gp_id_inverso = $diabetico_final['ine'];	
				$show_gp_nm_inverso = $diabetico_final['nome_equipe'];
				$show_gp_id2_inverso = $diabetico_final['cind_micro_area'];
				$show_gp_nm2_inverso = "Micro-&Aacute;rea";
			}
			...
			$show_gp_inverso = "Unidade";	
			$show_gp_id_inverso = $diabetico_final['cnes'];	
			$show_gp_nm_inverso = $diabetico_final['nome_unidade']." (".$diabetico_final['ine'].")";
			$show_gp_id2_inverso = $diabetico_final['cind_micro_area'];
			$show_gp_nm2_inverso = "Micro-&Aacute;rea";
			// ===========================================================================================================
			$rel_dados_grupo = "
		...
		if (pg_num_rows($run_sfm) <= 0){
			$campo_busca_res = "nu_cns_responsavel = '000000000000000'";
			if (strlen($diabetico_final['cns_resp']) == 15 && strlen($diabetico_final['cpf_resp']) == 11){
				$campo_busca_res = "(nu_cns_responsavel = '".$diabetico_final['cns_resp']."' OR nu_cpf_responsavel = '".$diabetico_final['cpf_resp']."')";
			} else {
				if (strlen($diabetico_final['cns_resp']) == 15){
					$campo_busca_res = "nu_cns_responsavel = '".$diabetico_final['cns_resp']."'";
				}
				if (strlen($diabetico_final['cpf_resp']) == 11){
					$campo_busca_res = "nu_cpf_responsavel = '".$diabetico_final['cpf_resp']."'";
				}
			}
			$familia = "
				SELECT 
		...
		if ($diabetico_final['cind_gestante'] == 1){
			$marcado_gestante = "SIM";
		}
		if ($diabetico_final['cind_hipertenso'] == 1){
			$marcado_hipertenso = "SIM";
		}
		if ($diabetico_final['cind_diabetico'] == 1){
		...
		if ($ma_familiar == '00' && $diabetico_final['cind_micro_area'] == '00'){
			$inconsistencias .= " diabético sem cadastro familiar.<br>";
		}

		if (substr($diabetico_final['cidadao_nome'],0,1) == '#'){
			$inconsistencias .= "diabético não localizada em tb_cidadao.<br>";
		}
		$inativo2 = 0;
		if ($diabetico_final['cidadao_ativo'] != 1){
			//$inconsistencias .= "Cadastro está como inativo (Cidadao).<br>";
			$inativo2++;
		}
		if ($diabetico_final['cind_inativo'] != 0){
		...
		$faleceu2 = 0;
		if ($diabetico_final['cidadao_faleceu'] != 0){
			$inconsistencias .= "Faleceu (Cidadao)<br>";
			$faleceu2++;
		}
		if (strlen($diabetico_final['data_falecimento']) > 5){
			$inconsistencias .= "Faleceu em ".dtshow($diabetico_final['data_falecimento'])."<br>";
			$faleceu2++;
		}
		if ($faleceu2 >= 2){
			$conta_geral_desconto++;
		...
		$idade = idadeint($diabetico_final['data_nascimento'],$dtf);
		...
		$rel_dados_unitario = "
		  <tr> 
			...
			<td colspan=\"5\" valign=\"top\" class=\"lista-dado1-esquerdo-B\">".$diabetico_final['cidadao_nome']."</td>
			<td valign=\"top\" class=\"lista-dado1-centro-B\">".dtshow($diabetico_final['data_nascimento'])."</td>
		  ...
		  <tr> 
			...
			<td colspan=\"7\" valign=\"top\" class=\"lista-dado2-centro-B\">".$diabetico_final['cidadao_mae']."</td>
		  </tr>
		  <tr> 
			<td height=\"18\"></td>
			<td></td>
		  ...
		  <tr> 
			...
			<td colspan=\"2\" valign=\"top\" class=\"lista-dado2-centro-B\">".$diabetico_final['cidadao_sexo']."</td>
		...
		$texto = zesq($conta_geral,5).";".mcpf($rCPF).";".mcns($rCNS).";".str_replace("Ã","A",$diabetico_final['cidadao_nome']).";".dtshow($diabetico_final['data_nascimento']).";".str_replace("Ã","A",$diabetico_final['cidadao_mae']).";".$idade.";".str_replace("Ã","A",$marcado_gestante).";".str_replace("Ã","A",$marcado_hipertenso).";".str_replace("Ã","A",$marcado_diabetico).";".str_replace("Ã","A",$exame_rotina).";".$diabetico_final['cidadao_sexo'].";".$diabetico_final['cnes'].";".$diabetico_final['ine'].";".$ma_familiar."/".$diabetico_final['cind_micro_area'].";".$conta_consultas_s1.";".$conta_consultas_s2.";".$soma_proc_s1.";".$soma_proc_s2.";".$apenas_solicitados."\r\n";
		fwrite($FT, $texto);
...
$link_pag = "";
if ($paginacao > 0){
	$pini = $pini + $num_diabeticos_final;
	$num = $numerador_ind7;
	$cdes = $conta_geral_desconto;
	if ($num_diabeticos_final > 0){
		$link_pag = "<a href=\"rel_diabeticos.php?pini=".$pini."&num=".$num."&cdes=".$cdes."\"><img src=\"images/pagina.png\"></a>";
	}
}
// -----------------------------------------------------------------------------------------------------------
...
// ===========================================================================================================
if ($paginacao <= 0 || $num_diabeticos_final == 0){
	$rel_indicadores = "
	</table>
	<table width=\"1009\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">
	  <!--DWLayoutTable-->
	  ...
	  <tr> 
		...
		<td valign=\"top\" class=\"resumo-v-numerador\">".$numerador_ind7."</td>
		<td></td>
	  </tr>
	  <tr> 
