...
fwrite($FT, $texto);
$texto = "Seq;CPF;CNS;DtConsulta;DUMdaCons;CNES;INE;CBO;CNSProf;NomeProf;ProfAlerta\r\n";
fwrite($FC, $texto);
$texto = "Seq;CPF;CNS;DtProced;DtCons;Tabela;CNSProf;NomeProf;CNES;INE;CBO;Exame;Procedimento(A/S);ProfAlerta\r\n";
fwrite($FP, $texto);
...
if ($num_getantes_final > 0){
	while ($gestante_final = pg_fetch_array($run_gf)){
		...
		$idade = idadeint($gestante_final['data_nascimento'],$dtf_idade);
		...
		*/
		...
		if ($mconsultas == 1){
			...
			for ($i=0;$i<$total_consultas;$i++){
				...
				$texto = zesq($soma_consultas,5).";".mcpf($rCPF).";".mcns($rCNS).";".dtshow($array_consultas[$i]['data']).";".dtshow($array_consultas[$i]['data_dum_consulta']).";".$array_consultas[$i]['cnes'].";".$array_consultas[$i]['ine'].";".$array_consultas[$i]['nu_cbo'].";".$array_consultas[$i]['cns_prof'].";".$array_consultas[$i]['no_profissional'].";".$profissionalAlerta."\r\n";
				fwrite($FC, $texto);
				$rel_dados_sub_tabela .= "
						<tr> 
		...
		if (count($exames) > 0){
			...
			for ($i=0;$i<count($exames);$i++){
				...
				$texto = $soma_procedimentos.";".mcpf($rCPF).";".mcns($rCNS).";".dtshow($exames[$i]['data']).";".$consproced_csv.";".$exames[$i]['tabela'].";".$exames[$i]['cns_prof'].";".$exames[$i]['no_profissional'].";".$exames[$i]['cnes'].";".$exames[$i]['ine'].";".$exames[$i]['cbo'].";".str_replace("í","i",$exames[$i]['tipo']).";".$exames[$i]['procedimento1']."/".$exames[$i]['procedimento2'].";".$profissionalAlerta."\r\n";
				fwrite($FP, $texto);
				$rel_dados_sub_tabela .= "
						<tr> 
