...
if (file_exists($_SESSION['filedb'])){
	...
	header('location:banco.php?ex=rel_criancas');
}
if (file_exists("cfg_rel_v.php")){
	require_once('cfg_rel_v.php');
} else {
	header('location:vacinacao.php?ex=rel_criancas');
}
...
// +++++++++++++++++++++++++++++++++++++++++++++
$file = "csv/crianca_T_01.csv";
if (file_exists($file)){unlink($file);}
$FT = fopen($file,'w');
$file = "csv/crianca_V_01.csv";
if (file_exists($file)){unlink($file);}
$FV = fopen($file,'w');
// +++++++++++++++++++++++++++++++++++++++++++++
...
<head>
  ...
  <title>Relatório de Crianças</title>
...
$num_criancas = 0;
$run_s1 = pg_query($cdb,$criancas);
$num_criancas = pg_num_rows($run_s1);
if ($num_criancas > 0){
	while ($crianca = pg_fetch_array($run_s1)){
		$CNS = '0';
		$CPF = '0';
		if (strlen(trim($crianca['nu_cns'])) == 15){
			$CNS = trim($crianca['nu_cns']);
		} else {
			if (strlen(trim($crianca['nu_cns_ci'])) == 15){
				$CNS = trim($crianca['nu_cns_ci']);
			}
		}
		if (strlen(trim($crianca['nu_cpf_cidadao'])) == 11){
			$CPF = trim($crianca['nu_cpf_cidadao']);
		} else {
			if (strlen(trim($crianca['nu_cpf_cidadao_ci'])) == 11){
				$CPF = trim($crianca['nu_cpf_cidadao_ci']);
			}
		}
		...
		if (!$duplicado){
			$data_nascimento = dataint($crianca['dt_nascimento']);
			$data_falecimento = "0";
			if (strlen(trim($crianca['dt_obito'])) > 5){
				$data_falecimento = dataint($crianca['dt_obito']);
			} else {
				if (strlen(trim($crianca['dt_obito_ci'])) > 5){
					$data_falecimento = dataint($crianca['dt_obito_ci']);
				}
			}
			$micro_area = $crianca['nu_micro_area'];
			...
			$micro_area_c = $crianca['nu_micro_area_c'];
			...
			$cns_resp = '0';
			$cpf_resp = '0';
			if (strlen($crianca['nu_cns_responsavel']) == 15){
				$cns_resp = $crianca['nu_cns_responsavel'];
			} else {
				$cns_resp = $crianca['nu_cns_responsavel_c'];
			}
			if (strlen($crianca['nu_cpf_responsavel']) == 11){
				$cpf_resp = $crianca['nu_cpf_responsavel'];
			} else {
				$cpf_resp = $crianca['nu_cpf_responsavel_c'];
			}

			/*
			$cidadao_nome = "# CRIANCA NÃO ENCONTRADO #";
			if (strlen(trim($crianca['no_cidadao'])) > 0){
				$cidadao_nome = trim($crianca['no_cidadao']);
			}
			*/
			
			/*
			$cidadao_mae = "# SEM NOME DA MAE #";
			if (strlen(trim($crianca['no_mae'])) > 0){
				$cidadao_mae = trim($crianca['no_mae']);
			}
			
			$cidadao_nome = htmlspecialchars(trim($crianca['no_cidadao']), ENT_QUOTES);
			...
			*/
			...
			//**********************************************************************************
			$nome_cidadao = $crianca['no_cidadao'];
			$nome_mae_cidadao = $crianca['no_mae'];
			//----------------------------------------------
			$cds = 0;
			...
			INSERT INTO tmp_criancas(
				...
				'".$cpf_resp."',
				".$crianca['st_gestante'].",
				".$crianca['st_hipertensao_arterial'].",
				".$crianca['st_diabete'].",
				'".$crianca['ds_sexo']."',
				'".$micro_area."',
				".$crianca['st_ficha_inativa'].",
				'".$crianca['nu_cnes']."',
				'".$crianca['no_unidade_saude']."',
				'".$ine."',
				'".$crianca['no_equipe']."',
				".$cds."
			)";
			...
			if ($super_filtro){
				if ($cpbusca == 'U'){
					...
					if ($crianca['nu_cnes'] == $vlbusca){
						$gravar = true;
					}
				}
				if ($cpbusca == 'E'){
					$vlbusca = zesq($vlbusca,10);
					if ($crianca['nu_ine'] == $vlbusca){
						$gravar = true;
					}
				}
				if ($cpbusca == 'M'){
...
$num_criancas_final = 0;
$run_mf = pg_query($cdb,$criancas_final);
$num_criancas_final = pg_num_rows($run_mf);
$conta_geral = $pini;
$conta_geral_desconto = $cdes;
$numerador_ind5 = $num;

if ($num_criancas_final > 0){
	while ($crianca_final = pg_fetch_array($run_mf)){
		$duplicados_vacinas = array();
		$conta_duplicados_vacinas = 0;
		
		$conta_geral++;
		
		$rCNS = trim($crianca_final['cns']);
		$rCPF = trim($crianca_final['cpf']);
		...
		if ($grupo != "SG"){
			$grupo_id = $crianca_final[$grupo];
			
			$grupo_nome = "Micro-Área";
			$show_gp_inverso = "Unidade";	
			$show_gp_id_inverso = $crianca_final['cnes'];	
			$show_gp_nm_inverso = $crianca_final['nome_unidade'];
			$show_gp_id2_inverso = $crianca_final['ine'];
			$show_gp_nm2_inverso = "Equipe";
			if ($grupo == "ine"){
				$grupo_nome = $crianca_final['nome_equipe'];	
				$show_gp_id2_inverso = $crianca_final['cind_micro_area'];
				$show_gp_nm2_inverso = "Micro-&Aacute;rea";
			}
			if ($grupo == "cnes"){
				$grupo_nome = $crianca_final['nome_unidade'];
				$show_gp_inverso = "Equipe";	
				$show_gp_id_inverso = $crianca_final['ine'];	
				$show_gp_nm_inverso = $crianca_final['nome_equipe'];
				$show_gp_id2_inverso = $crianca_final['cind_micro_area'];
				$show_gp_nm2_inverso = "Micro-&Aacute;rea";
			}
			...
			$show_gp_inverso = "Unidade";	
			$show_gp_id_inverso = $crianca_final['cnes'];	
			$show_gp_nm_inverso = $crianca_final['nome_unidade']." (".$crianca_final['ine'].")";
			$show_gp_id2_inverso = $crianca_final['cind_micro_area'];
			$show_gp_nm2_inverso = "Micro-&Aacute;rea";
			// ===========================================================================================================
			$rel_dados_grupo = "
		...
		if (pg_num_rows($run_sfm) <= 0){
			$campo_busca_res = "nu_cns_responsavel = '000000000000000'";
			if (strlen($crianca_final['cns_resp']) == 15 && strlen($crianca_final['cpf_resp']) == 11){
				$campo_busca_res = "(nu_cns_responsavel = '".$crianca_final['cns_resp']."' OR nu_cpf_responsavel = '".$crianca_final['cpf_resp']."')";
			} else {
				if (strlen($crianca_final['cns_resp']) == 15){
					$campo_busca_res = "nu_cns_responsavel = '".$crianca_final['cns_resp']."'";
				}
				if (strlen($crianca_final['cpf_resp']) == 11){
					$campo_busca_res = "nu_cpf_responsavel = '".$crianca_final['cpf_resp']."'";
				}
			}
			$familia = "
				SELECT 
		...
		if ($crianca_final['cind_gestante'] == 1){
			$marcado_gestante = "SIM";
		}
		if ($crianca_final['cind_hipertenso'] == 1){
			$marcado_hipertenso = "SIM";
		}
		if ($crianca_final['cind_diabetico'] == 1){
		...
		$faleceu2 = 0;
		if ($crianca_final['cidadao_faleceu'] != 0){
			$inconsistencias .= "Faleceu (Cidadao)<br>";
			$faleceu2++;
		}
		if (strlen($crianca_final['data_falecimento']) > 5){
			$inconsistencias .= "Faleceu em ".dtshow($crianca_final['data_falecimento'])."<br>";
			$faleceu2++;
		}
		if ($faleceu2 >= 2){
			$conta_geral_desconto++;
		...
		$idade = idadeint($crianca_final['data_nascimento'],$dtf);
		if ($idade <= 0 && substr($crianca_final['data_nascimento'],-4) > date('md')){
			$dta_sub = date('Y').substr($crianca_final['data_nascimento'],4,4);
		...
		$rel_dados_unitario = "
		  <tr> 
			...
			<td colspan=\"5\" valign=\"top\" class=\"lista-dado1-esquerdo-B\">".$crianca_final['cidadao_nome']."</td>
			<td valign=\"top\" class=\"lista-dado1-centro-B\">".dtshow($crianca_final['data_nascimento'])."</td>
		  ...
		  <tr> 
			...
			<td colspan=\"2\" valign=\"top\" class=\"lista-sub-dados-BB\">Nome da m&atilde;e (".$crianca_final['cds'].")</td>
			<td colspan=\"7\" valign=\"top\" class=\"lista-dado2-centro-B\">".$crianca_final['cidadao_mae']."</td>
		  </tr>
		  <tr> 
			<td height=\"18\"></td>
			<td></td>
		...
		$texto = zesq($conta_geral,5).";".mcpf($rCPF).";".mcns($rCNS).";".str_replace("Ã","A",$crianca_final['cidadao_nome']).";".dtshow($crianca_final['data_nascimento']).";".str_replace("Ã","A",$crianca_final['cidadao_mae']).";".$idade.";".str_replace("Ã","A",$marcado_gestante).";".str_replace("Ã","A",$marcado_hipertenso).";".str_replace("Ã","A",$marcado_diabetico).";".str_replace("Ã","A",$vacina_citopatologico).";".$crianca_final['cnes'].";".$crianca_final['ine'].";".$ma_familiar."/".$crianca_final['cind_micro_area']."\r\n";
		fwrite($FT, $texto);
...
$link_pag = "";
if ($paginacao > 0){
	$pini = $pini + $num_criancas_final;
	$num = $numerador_ind5;
	$cdes = $conta_geral_desconto;
	if ($num_criancas_final > 0){
		$link_pag = "<a href=\"rel_criancas.php?pini=".$pini."&num=".$num."&cdes=".$cdes."\"><img src=\"images/pagina.png\"></a>";
	}
}
// -----------------------------------------------------------------------------------------------------------
...
// ===========================================================================================================
if ($paginacao <= 0 || $num_criancas_final == 0){
	$rel_indicadores = "
	</table>
	<table width=\"1009\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">
	  <!--DWLayoutTable-->
	  ...
	  <tr> 
		...
		<td valign=\"top\" class=\"resumo-v-numerador\">".$numerador_ind5."</td>
		<td></td>
	  </tr>
	  <tr> 
