...
$FT = fopen($file,'w');
$file = "csv/covid_V_01.csv";
if (file_exists($file)){unlink($file);}
$FV = fopen($file,'w');
// +++++++++++++++++++++++++++++++++++++++++++++
$texto = "Seq;CPF;CNS;Nome;DtNascimento;Mae;Idade;MarcGestante;MarcHipertensa;MarcDiabetica;CNES;INE;MA\r\n";
fwrite($FT, $texto);
$texto = "Seq;CPF;CNS;DtProced;Dose;CNES;INE;CBO;Imunobiologico;Lote;Fabricante;CNSProf;NomeProf;ImunoSigla;ImunoNome\r\n";
fwrite($FV, $texto);
...
if ($num_vacinados_final > 0){
	while ($vacinado_final = pg_fetch_array($run_mf)){
		...
		*/
		...
		$idade = idadeint($vacinado_final['data_nascimento']);
		...
		if (count($vacinas) > 0){
			...
			for ($i=0;$i<count($vacinas);$i++){
				...
				$texto = $soma_vacinas.";".mcpf($rCPF).";".mcns($rCNS).";".dtshow($vacinas[$i]['data']).";".$vacinas[$i]['dose'].";".$vacinas[$i]['cnes'].";".$vacinas[$i]['ine'].";".$vacinas[$i]['cbo'].";".$vacinas[$i]['imuno'].";".$vacinas[$i]['lote'].";".$vacinas[$i]['fabricante'].";".$vacinas[$i]['nu_cns'].";".$vacinas[$i]['no_profissional'].";".$vacinas[$i]['imsg'].";".$vacinas[$i]['imnome']."\r\n";
				fwrite($FV, $texto);
