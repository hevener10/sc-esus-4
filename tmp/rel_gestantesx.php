...
$FP = fopen($file,'w');
// +++++++++++++++++++++++++++++++++++++++++++++
$texto = "Seq;CPF;CNS;Nome;DtNascimento;Mae;Idade;MarcGestante;MarcHipertensa;MarcDiabetica;Interrupcao;DUM;20s;DPP;FimPuerperio;NumConsultas;DtPrimConsulta;NumConsOdonto;Indicador1;Indicador2;Indicador3;Sexo;CNES;INE;MA\r\n";
fwrite($FT, $texto);
$texto = "Seq;CPF;CNS;DtConsulta;DUMdaCons;CNES;INE;CBO\r\n";
fwrite($FC, $texto);
$texto = "Seq;CPF;CNS;DtProced;DtCons;Tabela;CNES;INE;CBO;Exame;Procedimento(A/S)\r\n";
fwrite($FP, $texto);
		...
		if ($busca){
			...
			if (!$duplicado){
				...
				/*
					...
					$CPF # varchar(11)
					$data_nascimento # bigint
					$gestante['co_dim_tempo_dum'] # bigint
					$gestante['co_dim_tempo'] # bigint
					$cidadao_ativo # int
					$data_falecimento # bigint
					$cidadao_nome # varchar(500)
					$cidadao_mae # varchar(500)
					$cidadao_faleceu # int (normal é 0)
					$cidadao_sexo # varchar(24)
					$cind_gestante # int (normal NULL ou 0)
					$cind_hipertenso # int (normal NULL ou 0)
					$cind_diabetico # int (normal NULL ou 0)
					$cind_micro_area # varchar(3)
					$cind_inativo # int (normal é 0)
					$cnes # varchar(20)
					$nome_unidade # varchar(500)
					$ine # varchar(20)
					$nome_equipe # varchar(255)
...
if ($num_getantes_final > 0){
	while ($gestante_final = pg_fetch_array($run_gf)){
		...
		/*
		$gestante_final['sequencia']
		$gestante_final['cns']
		$gestante_final['cpf']
		$gestante_final['data_nascimento']
		$gestante_final['g_co_dim_tempo_dum']
		$gestante_final['g_co_dim_tempo']
		$gestante_final['cidadao_ativo']
		$gestante_final['data_falecimento']
		$gestante_final['cidadao_nome']
		$gestante_final['cidadao_mae']
		$gestante_final['cidadao_faleceu']
		$gestante_final['cidadao_sexo']
		$gestante_final['cind_gestante']
		$gestante_final['cind_hipertenso']
		$gestante_final['cind_diabetico']
		$gestante_final['cind_micro_area']
		$gestante_final['cind_inativo']
		$gestante_final['cnes']
		$gestante_final['nome_unidade']
		$gestante_final['ine']
		$gestante_final['nome_equipe']
		$gestante_final['calc_ddp']
		$gestante_final['calc_20s']
		$gestante_final['calc_fimpuerp']
		*/
		$conta_geral++;
		...
		$interrupcoes = $interrupcoes.$lista_inter."
							ORDER BY co_dim_tempo DESC
							LIMIT 1
						) AS t1
						LEFT JOIN
		...
		for ($v=0;$v<2;$v++){
			...
			if (pg_num_rows($run_st2) > 0){
				while ($ex2 = pg_fetch_array($run_st2)){
					$junta_exames = $ex2['co_dim_tempo'].$ex2['nu_cbo'].$ex2['nu_cnes'].$ex2['nu_ine'].$ex2['procedimento'].'0';
					if (!in_array($junta_exames, $duplicados_exames)) {
						$duplicados_exames[$conta_duplicados_exames] = $junta_exames;
						$conta_duplicados_exames++;
						...
						$exames[$cont_exames]['procedimento2'] = '0';
		...
		if ($num_interrupcoes > 0){
			if ($data_interrupcao < $dti){
				$conta_geral_desconto++;
				$inconsistencias .= "Teve inter. de gestação em: ".dtshow($data_interrupcao)."<br>-<br>";
			} else {
				$inconsistencias .= "Teve inter. de gestação em: ".dtshow($data_interrupcao)."<br>";
			}
		}
		...
		$idade = idadeint($gestante_final['data_nascimento']);
		...
		if ($num_trimestres > 0){
			if (strlen($idade_gest) > 0){
				$idade_gest .= "<br>";
			}
			$idade_gest .= "!! É necessário ter ao menos ".$num_trimestres." consultas !!";
		}
		// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		$consulta_odontologica = "NÃO";
		$estilo_odonto = "indicador-c1-NAO";
		$estilo_odonto_indicador = "indicador-titulo-Vermelho";
		if ($I3_c1){
			$consulta_odontologica = "SIM";
			$estilo_odonto = "indicador-c1-SIM";
			$estilo_odonto_indicador = "indicador-titulo-Verde";
			$numerador_ind3++;
			if ($num_trimestres != $num_codontos){
				$consulta_odontologica = "SIM";
				$estilo_odonto = "indicador-c1-SIM-amarelo";
			}
		}
		...
		$rel_dados_unitario = "
		  ...
		  <tr> 
			...
			<td colspan=\"3\" valign=\"top\" class=\"lista-sub-dados-B\">Interrup&ccedil;&atilde;o Cl&iacute;nica </td>
		  ...
		  <tr> 
			...
			<td colspan=\"4\" rowspan=\"2\" valign=\"top\" class=\"indicador-c1-centro-X\">.</td>
		  ...
		  <tr> 
			...
			<td colspan=\"5\" rowspan=\"2\" valign=\"top\" class=\"indicador-c1\">Crit&eacute;rio 1: Consulta odontol&oacute;gica</td>
		  ...
		  <tr> 
			<td height=\"16\"></td>
			<td></td>
			<td colspan=\"2\" valign=\"top\" class=\"indicador-cx\">".$marcado_gestante_odonto_show."</td>
		  </tr>";
		$linhas = $linhas + 13;
		...
		$texto = zesq($conta_geral,5).";".mcpf($rCPF).";".mcns($rCNS).";".str_replace("Ã","A",$gestante_final['cidadao_nome']).";".dtshow($gestante_final['data_nascimento']).";".str_replace("Ã","A",$gestante_final['cidadao_mae']).";".$idade.";".str_replace("Ã","A",$marcado_gestante).";".str_replace("Ã","A",$marcado_hipertenso).";".str_replace("Ã","A",$marcado_diabetico).";".str_replace("Ã","A",$interrup_show).";".dtshow($gestante_final['g_co_dim_tempo_dum']).";".dtshow($gestante_final['calc_20s']).";".dtshow($gestante_final['calc_ddp']).";".dtshow($gestante_final['calc_fimpuerp']).";".$total_consultas.";".dtshow($dt_primeira_consulta).";".$num_codontos.";".str_replace("Ã","A",$indicador1).";".str_replace("Ã","A",$show_indicador_2).";".str_replace("Ã","A",$consulta_odontologica).";".$gestante_final['cidadao_sexo'].";".$gestante_final['cnes'].";".$gestante_final['ine'].";".$ma_familiar."/".$gestante_final['cind_micro_area']."\r\n";
		fwrite($FT, $texto);
		
		if ($mconsultas == 1){
			$rel_dados_unitario_sub_cab = "
			  <tr> 
				<td height=\"19\"></td>
				<td></td>
				<td colspan=\"2\" valign=\"top\" class=\"consultas-titulo\">Consultas</td>
				<td colspan=\"4\" valign=\"top\" class=\"consultas-cabecado\">Data</td>
				<td colspan=\"3\" valign=\"top\" class=\"consultas-cabecado\">DUM informado</td>
				<td colspan=\"5\" valign=\"top\" class=\"consultas-cabecado\">INE | CBO Profissional de atendimento</td>
			  </tr>
			";
			echo $rel_dados_unitario_sub_cab;
			
			// ===========================================================================================================
			
			$conta_linhas_unit_sub = 0;
			for ($i=0;$i<$total_consultas;$i++){
				
				usort($array_consultas, 'cmp');
				
				$mostra_conta = $i + 1;
				$texto = zesq($mostra_conta,5).";".mcpf($rCPF).";".mcns($rCNS).";".dtshow($array_consultas[$i]['data']).";".dtshow($array_consultas[$i]['data_dum_consulta']).";".$array_consultas[$i]['cnes'].";".$array_consultas[$i]['ine'].";".$array_consultas[$i]['nu_cbo']."\r\n";
				fwrite($FC, $texto);

				// ===========================================================================================================
				$rel_dados_unitario_sub = "
				  <tr> 
					<td height=\"19\"></td>
					<td></td>
					<td></td>
					<td width=\"86\" class=\"consultas-contador\">".$mostra_conta."&nbsp;&nbsp;</td>
					<td colspan=\"3\" valign=\"top\" class=\"consultas-dados-centro-B\">".dtshow($array_consultas[$i]['data'])."</td>
					<td width=\"17\" valign=\"top\" class=\"consultas-dados-centro\">".$array_consultas[$i]['indicativo_consulta']."</td>
					<td colspan=\"2\" valign=\"top\" class=\"consultas-dados-centro\">".dtshow($array_consultas[$i]['data_dum_consulta'])."</td>
					<td width=\"17\" valign=\"top\" class=\"consultas-dados-centro\">".$array_consultas[$i]['indicativo_dum']."</td>
					<td width=\"65\" valign=\"top\" class=\"consultas-dados-centro\">".$array_consultas[$i]['cnes']."</td>
					<td colspan=\"4\" valign=\"top\" class=\"consultas-dados-esquerda\">".$array_consultas[$i]['ine']." | ".$array_consultas[$i]['no_cbo']."</td>
				  </tr>
				";
				$conta_linhas_unit_sub++;
				echo $rel_dados_unitario_sub;
				// ===========================================================================================================

			}
			unset($array_consultas);
			$linhas = $linhas + $conta_linhas_unit_sub;
			if ($total_consultas <= 0){
				$rel_dados_unitario_sub = "
				  <tr> 
					...
					<td></td>
					<td width=\"86\"></td>
					<td colspan=\"3\" valign=\"top\" class=\"consultas-dados-centro-B\">".dtshow($gestante_final['g_co_dim_tempo'])."</td>
					<td width=\"17\" valign=\"top\" class=\"consultas-dados-centro\"></td>
					<td colspan=\"2\" valign=\"top\" class=\"consultas-dados-centro\"></td>
					<td width=\"17\" valign=\"top\" class=\"consultas-dados-centro\"></td>
					<td width=\"65\" valign=\"top\" class=\"consultas-dados-centro\"></td>
					<td colspan=\"4\" valign=\"top\" class=\"consultas-dados-esquerda\">SEM CONSULTA DE PRÉ-NATAL</td>
				  </tr>
				";
				$linhas++;
				echo $rel_dados_unitario_sub;
		...
		if (count($exames) > 0){
			...
			for ($i=0;$i<count($exames);$i++){
				...
				$texto = $soma_procedimentos.";".mcpf($rCPF).";".mcns($rCNS).";".dtshow($exames[$i]['data']).";".$consproced_csv.";".$exames[$i]['tabela'].";".$exames[$i]['cnes'].";".$exames[$i]['ine'].";".$exames[$i]['cbo'].";".str_replace("í","i",$exames[$i]['tipo']).";".$exames[$i]['procedimento1']."/".$exames[$i]['procedimento2']."\r\n";
				fwrite($FP, $texto);
