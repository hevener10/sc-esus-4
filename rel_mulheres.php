...
if (file_exists($_SESSION['filedb'])){
	...
	header('location:banco.php?ex=rel_mulheres');
}
if (file_exists("cfg_rel_m.php")){
	require_once('cfg_rel_m.php');
} else {
	header('location:mulheres.php?ex=rel_mulheres');
}
...
$cst = isset($_GET["cst"]) ? trim($_GET["cst"]) : 0;

// +++++++++++++++++++++++++++++++++++++++++++++
$file = "csv/mulher_T_01.csv";
if (file_exists($file)){unlink($file);}
$FT = fopen($file,'w');
$file = "csv/mulher_P_01.csv";
if (file_exists($file)){unlink($file);}
$FP = fopen($file,'w');
// +++++++++++++++++++++++++++++++++++++++++++++
...
if ($num_mulheres_final > 0){
	while ($mulher_final = pg_fetch_array($run_mf)){
		...
		*/
		...
		$idade = idadeint($mulher_final['data_nascimento'],$data_padrao);
		if ($idade < 28){
			$descano = 0;
			if ($idade == 25){
				$descano = 0;
			...
			$comp_dt_an = substr($mulher_final['data_nascimento'],4,4);
			if ($dt3anos == 'P'){
				...
				if (!in_array($junta_exames, $duplicados_exames)) {
					...
					if ($cst == 1){
						$consulta = "
							...
							WHERE 
								".$campo_busca." AND 
								co_dim_tempo = ".$ex1['co_dim_tempo']."
							LIMIT 1
						";
						$run_cons = pg_query($cdb,$consulta);
						if (pg_num_rows($run_cons) > 0){
							$consultas[$cont_consultas]['data'] = $ex1['co_dim_tempo'];
							$consultas[$cont_consultas]['cids'] = str_replace("|"," ",pg_fetch_result($run_cons,0,'ds_filtro_cids'));
							$consultas[$cont_consultas]['ciaps'] = str_replace("|"," ",pg_fetch_result($run_cons,0,'ds_filtro_ciaps'));
							$cont_consultas++;
						}
					}
				}
				...
				if (!in_array($junta_exames, $duplicados_exames)) {
					...
					if ($cst == 1){
						$consulta = "
							...
							WHERE 
								".$campo_busca." AND 
								co_dim_tempo = ".$ex2['co_dim_tempo']."
							LIMIT 1
						";
						$run_cons = pg_query($cdb,$consulta);
						if (pg_num_rows($run_cons) > 0){
							$consultas[$cont_consultas]['data'] = $ex2['co_dim_tempo'];
							$consultas[$cont_consultas]['cids'] = str_replace("|"," ",pg_fetch_result($run_cons,0,'ds_filtro_cids'));
							$consultas[$cont_consultas]['ciaps'] = str_replace("|"," ",pg_fetch_result($run_cons,0,'ds_filtro_ciaps'));
							$cont_consultas++;
						}
		...
		$texto = zesq($conta_geral,5).";".mcpf($rCPF).";".mcns($rCNS).";".str_replace("Ã","A",$mulher_final['cidadao_nome']).";".dtshow($mulher_final['data_nascimento']).";".str_replace("Ã","A",$mulher_final['cidadao_mae']).";".$idade.";".str_replace("Ã","A",$marcado_gestante).";".str_replace("Ã","A",$marcado_hipertenso).";".str_replace("Ã","A",$marcado_diabetico).";".str_replace("Ã","A",$exame_citopatologico).";".$mulher_final['cnes'].";".$mulher_final['ine'].";".$ma_familiar."/".$mulher_final['cind_micro_area']."\r\n";
		fwrite($FT, $texto);
		...
		if (count($exames) > 0){
			...
			for ($i=0;$i<count($exames);$i++){
				...
				$texto = $soma_procedimentos.";".mcpf($rCPF).";".mcns($rCNS).";".dtshow($exames[$i]['data']).";".$exames[$i]['tabela'].";".$exames[$i]['cns_prof'].";".$exames[$i]['no_profissional'].";".$exames[$i]['cnes'].";".$exames[$i]['ine'].";".$exames[$i]['cbo'].";".$exames[$i]['procedimento1']."/".$exames[$i]['procedimento2'].";".$profissionalAlerta."\r\n";
				fwrite($FP, $texto);
				$rel_dados_sub_tabela .= "
						<tr> 
				...
				for ($i=0;$i<count($consultas);$i++){
					...
					$rel_dados_sub_tabela .= "
							<tr> 
							  ...
							  <td valign=\"top\" class=\"procedimentos-dado\">".$consultas[$i]['cids']."</td>
							  <td valign=\"top\" class=\"procedimentos-dado\">".dtshow($consultas[$i]['data'])."</td>
							  <td valign=\"top\" class=\"procedimentos-dado\">".$consultas[$i]['ciaps']."</td>
...
if ($paginacao > 0){
	...
	if ($num_mulheres_final > 0){
		$link_pag = "<a href=\"rel_mulheres.php?pini=".$pini."&num=".$num."&cdes=".$cdes."\"><img src=\"images/pagina.png\"></a>";
	}
}
// -----------------------------------------------------------------------------------------------------------
...
// ===========================================================================================================
if (($paginacao <= 0 && $ridade <= 0) || $num_mulheres_final == 0){
	$rel_indicadores = "
	</table>
	<table width=\"1009\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">
	  <!--DWLayoutTable-->
