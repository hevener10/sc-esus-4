...
<head>
  ...
  <title>Configura&ccedil;&otilde;es do relat&oacute;rio de Vacinação</title>
  <link href="css/form.css" rel="stylesheet" type="text/css">
</head>

<body>
<form method="post" action="relatorios.php" onsubmit="return validateForm()" id="formv" name="formv" >
  <input type="hidden" id="gravarva" name="gravarva" value="1">
  <input type="hidden" id="ex" name="ex" value="<?php echo $ex;?>">
  <table width="587" border="0" cellpadding="0" cellspacing="0">
    <!--DWLayoutTable-->
    <tr> 
      <td height="33" colspan="2" valign="top" class="titulo">Configura&ccedil;&otilde;es do relat&oacute;rio de Vacinação</td>
    </tr>
    <tr> 
    ...
    <tr> 
      <td height="27" valign="top" class="rotulo">(*) Imunobiológicos&nbsp;&nbsp;</td>
      <td valign="top" class="valor">&nbsp;&nbsp;<input type="text" name="imbios" id="imbios" value="<?php echo $imbios;?>"> </td>
    </tr>
    <tr> 
      <td height="27" valign="top" class="rotulo">(*) Apenas vacinados&nbsp;&nbsp;</td>
    ...
    <tr> 
      <td height="35" colspan="2" valign="top" class="rodape"><button onclick="window.location.href='relatorios.php'">Cancelar</button> | <button type="submit" form="formv" value="Submit">Gravar</button></td>
    </tr>
    <tr> 
...
function validateForm() {
	...
	var ctrld = 0;
	if (!validaData(document.forms["formv"]["dti"].value)){
		mensagem = mensagem + "Campo 'Data inicial' não é uma data válida!<br>";
		submit = false;
		ctrld = 1;
	}
	if (!validaData(document.forms["formv"]["dtf"].value)){
		mensagem = mensagem + "Campo 'Data final' não é uma data válida!<br>";
		submit = false;
		ctrld = 1;
	}
	if (ctrld == 0){
		var dti = parseInt(document.forms["formv"]["dti"].value.substring(6,10) + document.forms["formv"]["dti"].value.substring(3,5) + document.forms["formv"]["dti"].value.substring(0,2));
		var dtf = parseInt(document.forms["formv"]["dtf"].value.substring(6,10) + document.forms["formv"]["dtf"].value.substring(3,5) + document.forms["formv"]["dtf"].value.substring(0,2));
		var gpa = document.forms["formv"]["gpa"].value;
		if (gpa == 0){
			var ctrlp = 0;
			if (isNaN(dti)){
