...
<head>
  ...
  <title>Configura&ccedil;&otilde;es do relat&oacute;rio de diabéticos</title>
  <link href="css/form.css" rel="stylesheet" type="text/css">
</head>

<body>
<form method="post" action="relatorios.php" onsubmit="return validateForm()" id="formd" name="formd" >
  <input type="hidden" id="gravardi" name="gravardi" value="1">
  <input type="hidden" id="ex" name="ex" value="<?php echo $ex;?>">
  <table width="587" border="0" cellpadding="0" cellspacing="0">
    <!--DWLayoutTable-->
    <tr> 
      <td height="33" colspan="2" valign="top" class="titulo">Configura&ccedil;&otilde;es do relat&oacute;rio de diabéticos</td>
    </tr>
    <tr> 
...
function validateForm() {
	...
	var ctrld = 0;
	if (!validaData(document.forms["formd"]["dti"].value)){
		mensagem = mensagem + "Campo 'Data inicial' não é uma data válida!<br>";
		submit = false;
		ctrld = 1;
	}
	if (!validaData(document.forms["formd"]["dtf"].value)){
		mensagem = mensagem + "Campo 'Data final' não é uma data válida!<br>";
		submit = false;
		ctrld = 1;
	}
	if (ctrld == 0){
		var dti = parseInt(document.forms["formd"]["dti"].value.substring(6,10) + document.forms["formd"]["dti"].value.substring(3,5) + document.forms["formd"]["dti"].value.substring(0,2));
		var dtf = parseInt(document.forms["formd"]["dtf"].value.substring(6,10) + document.forms["formd"]["dtf"].value.substring(3,5) + document.forms["formd"]["dtf"].value.substring(0,2));
		var gpa = document.forms["formd"]["gpa"].value;
		if (gpa == 0){
			var ctrlp = 0;
			if (isNaN(dti)){
